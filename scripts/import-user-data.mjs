...
async function showUserStats(userLogin) {
  try {
    ...
    const { data: userStats, error: statsError } = await supabase
      .rpc('get_user_stats', { target_user_login: userLogin })
    ...
    if (userStats && userStats.length > 0) {
      ...
      console.log(`📅 账号创建: ${new Date(stats.account_created_at).toLocaleDateString()}`)
    }

    // 获取语言统计
    const { data: languageStats, error: langError } = await supabase
      .rpc('get_language_stats_by_user', { target_user_login: userLogin })

    if (!langError && languageStats && languageStats.length > 0) {
      console.log('\n🔤 编程语言统计:')
      languageStats.slice(0, 10).forEach((lang, index) => {
        console.log(`${index + 1}. ${lang.language}: ${lang.repo_count} 个仓库 (⭐${lang.total_stars})`)
      })
    }
...
async function main() {
  ...
  if (args.length < 2) {
    console.log('使用方法: node import-user-data.mjs <用户信息JSON路径> <仓库数据JSON路径>')
    console.log('例如: node import-user-data.mjs "../data/google-gemini_user_info.json" "../data/google-gemini_repositories.json"')
    process.exit(1)
  }
  ...
  console.log('\n🎉 导入完成!')
  console.log(`📊 最终统计: 用户信息已导入，仓库 ${repoStats.successCount}/${repoStats.total} 条记录导入成功`)
}
